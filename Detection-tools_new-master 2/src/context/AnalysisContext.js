  ...
  const api = {
    ...
    startAnalysis: async (fileId) => {
      try {
        ...
        dispatch({
          ...
          payload: { file_id: fileId, status: "processing" },
        });
        toast.success("Analysis started!");
        return response.data;
      } catch (error) {
        const errorMessage =
          error.response?.data?.detail || "Analysis failed to start";
        dispatch({ type: ActionTypes.SET_ERROR, payload: errorMessage });
        toast.error(errorMessage);
        throw error;
      } finally {
    ...
    getResults: async (fileId) => {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/results/${fileId}`,
          {},
        );
        return response.data;
      } catch (error) {
        const errorMessage =
          error.response?.data?.detail || "Failed to get results";
        dispatch({ type: ActionTypes.SET_ERROR, payload: errorMessage });
        throw error;
      }
    },
    ...
    listFiles: async () => {
      try {
        ...
        dispatch({ type: ActionTypes.SET_LOADING, payload: false });
        const errorMessage =
          error.response?.data?.detail || "Failed to list files";
        dispatch({ type: ActionTypes.SET_ERROR, payload: errorMessage });
    ...
    getEducationalContent: async () => {
      try {
        ...
        const errorMessage =
          error.response?.data?.detail || "Failed to get educational content";
        dispatch({ type: ActionTypes.SET_ERROR, payload: errorMessage });
        throw error;
      }
    },
    ...
    const poll = async () => {
      try {
        ...
        if (result.status === "completed") {
          ...
          dispatch({
            type: ActionTypes.SET_CURRENT_ANALYSIS,
          ...
          dispatch({
            type: ActionTypes.UPDATE_FILE,
            payload: { file_id: fileId, status: "error", error: result.error },
          });
        ...
        if (attempts < maxAttempts) {
          ...
          dispatch({
            type: ActionTypes.UPDATE_FILE,
            payload: { file_id: fileId, status: "timeout" },
          });
